...
export default () => {
  ...
  return (
    <>
      <h1 className="mb-4 text-3xl md:text-5xl font-bold">Lyricist's Room</h1>
      <input 
        placeholder="키워드를 입력해주세요.. 한 단어일수록 좋아요!" value={topic} onChange={e => setTopic(e.target.value)}
        className="w-5/6 md:w-[450px] h-14 p-4 text-sm md:text-lg border rounded-lg"
      />
      <input
      ...
      <select value={mood} onChange={e => setMood(e.target.value)}
        className="w-5/6 md:w-[450px] h-14 p-4 text-base md:text-lg border rounded-lg"
        >
